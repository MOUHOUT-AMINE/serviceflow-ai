...
describe('role workflows', () => {
  ...
  it('shows admin navigation and assigns a request', async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText('Attribuer à un agent actif'), '2')
...
describe('AI ticket suggestions', () => {
  it('shows loading and renders suggestions for review', async () => {
    ...
    renderApp(<TestRoutes />, '/service-requests')
    await userEvent.click(await screen.findByRole('button', { name: 'Printer offline' }))
    await userEvent.click(screen.getByRole('button', { name: 'Générer les suggestions' }))
    ...
    expect(screen.getByText('Généré par l’IA · À vérifier avant application')).toBeVisible()
  ...
  it('shows a small fallback when AI is unavailable', async () => {
    ...
    await userEvent.click(await screen.findByRole('button', { name: 'Printer offline' }))
    await userEvent.click(screen.getByRole('button', { name: 'Générer les suggestions' }))
    expect(await screen.findByText('Les suggestions de l’IA sont temporairement indisponibles.')).toBeVisible()
    expect(screen.getByRole('button', { name: 'Fait' })).toBeEnabled()
  })
  ...
  it('ignores a completed suggestion request after switching tickets', async () => {
    ...
    releaseFirst?.()
    await waitFor(() => expect(screen.getByRole('button', { name: 'Générer les suggestions' })).toBeEnabled())
...
describe('AI suggestion actions', () => {
  ...
  it('shows a retryable error for other generation failures', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Générer les suggestions' })).toBeEnabled()
  })
})
